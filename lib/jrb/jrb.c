...
static void insert(JRB item, JRB list) {
  JRB last_node;
...
static void delete_item(JRB item) {
  ...
  if (ishead(t) && t->parent == t) {
    ...
    if (isext(t)) {
      ...
      printf("Int node 0x%x: %c,%c: l=0x%x, r=0x%x, p=0x%x, lr=(%s,%s)\n", t, isred(t) ? 'R' : 'B',
             isleft(t) ? 'l' : 'r', t->flink, t->blink, t->parent, getlext(t)->key.s,
             getrext(t)->key.s);
    }
  }
}
  ...
  if (ishead(t) && t->parent == t) {
    ...
    if (isext(t)) {
      ...
      printf("Ext node 0x%x: %c,%c: p=0x%x, <=0x%x, >=0x%x k=%d\n", t, isred(t) ? 'R' : 'B',
             isleft(t) ? 'l' : 'r', t->parent, t->blink, t->flink, t->key.i);
    } else {
      ...
      printf("Int node 0x%x: %c,%c: l=0x%x, r=0x%x, p=0x%x, lr=(%d,%d)\n", t, isred(t) ? 'R' : 'B',
             isleft(t) ? 'l' : 'r', t->flink, t->blink, t->parent, getlext(t)->key.i,
             getrext(t)->key.i);
    }
  }
}
...
static JRB jrb_insert_a(JRB nd, Jval key, Jval val) { return jrb_insert_b(nd->flink, key, val); }
