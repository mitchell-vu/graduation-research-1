 ...
 * the first item in list l is l->flink.
...
void dll_insert_before(Dllist node, Jval v) /* Inserts before a given node */
{
  Dllist newnode;
...
void dll_insert_after(Dllist n, Jval val) /* Inserts after a given node */
{
  dll_insert_before(n->flink, val);
}

void dll_append(Dllist l, Jval val) /* Inserts at the end of the list */
{
  dll_insert_before(l, val);
}

void dll_prepend(Dllist l, Jval val) /* Inserts at the beginning of the list */
{
  dll_insert_before(l->flink, val);
}
